...
def open(level_code: str, v4: bool = True) -> Level:
    if re.match(r"^V1;\d+;\d+;(\d\.\d,?)*;([0-8]\.[0-3]\.\d+\.\d+,?)*;[\w\d]*;[0-3]?$", level_code):
        level_code = level_code.split(";")
        level = Level(int(level_code[1]), int(level_code[2]), level_code[5], int(level_code[6]) if v4 else 0)
        ...
        for i in level_code[4].split(","):
            ...
            level[int(x), level.height - int(y) - 1, False] = Cell(int(type), int(rotation))
...
def save(level: Level, v4: bool = True) -> str:
    ...
    for x, y, cell, place in level:
        ...
        if cell.type != 0:
            cells.append(f"{int(cell.type)}.{int(cell.rotation)}.{x}.{level.height - y - 1}")
